...
parser = argparse.ArgumentParser(
    description="Convert vtk to glb for the Millitome use case"
)
parser.add_argument(
    "--vtk",
    dest="path_to_vtk",
...
if Path(args.path_to_glb).suffix != ".glb":
    raise ValueError(
        "Please include the filename with the .glb extension as part of the path you provide"
    )
...
block_indices = [
    np.flatnonzero(np.all(mesh.visual.vertex_colors == block_color, axis=1))
    for block_color in block_colors
]

# extract the faces corresponding to each block
block_faces = [
    np.any(np.isin(mesh.faces, indices), axis=1) for indices in block_indices
]
