...
class NonHRAMapping:
    ...
    """
    def __init__( self, stage1_projection: Path, config_path: Path, output_dir: Path, ontology_term: str):
    ...
    def fetch_and_filter(self, result):
        ...
        try:
            ...
            filter_result = filter_samples(deepcopy(self.data[ConfigKeys.AT_GRAPH]), organ)
    ...
    def load_projection_block(self):
        ...
        self.projected_blocks = ProjectionBlockGenerator(self.stage1_projection, self.config_path).generate_projections()

        # Extract oriented bounding boxes for each block
        self.projected_blocks_obb = {id: block.bounding_box_oriented for id, block in deepcopy(self.projected_blocks).items()}

        for index, obb in self.projected_blocks_obb.items():
            obb.visual.vertex_colors = self.projected_blocks[index].visual.vertex_colors[0]
    ...
    def build_model(self, result):
        ...
        flip_vec = self.config_dict[ConfigKeys.NON_HRA_MAPPING].get(ConfigKeys.FLIP, [1, -1, -1])
        if len(flip_vec) != 3:
            raise ValueError("non_hra_mapping.flip must be length 3")

        flip = np.diag(
            [flip_vec[0], flip_vec[1], flip_vec[2], 1.0]
        )

        angle_rad = np.deg2rad(self.config_dict[ConfigKeys.NON_HRA_MAPPING][ConfigKeys.ROTATION_Y])
        ...
        scale_millitome_block(self.blocks, self.config_dict[ConfigKeys.NON_HRA_MAPPING][ConfigKeys.MODEL_SCALE])
