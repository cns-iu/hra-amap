...
def main():
    parser = argparse.ArgumentParser(description="Convert folder of .stl files to single .glb")
    parser.add_argument("--stl_dir", required=True, help="Directory containing .stl files")
    parser.add_argument("--glb", required=True, help="Path to output .glb file (.glb extension required)")
    ...
    if not stl_dir.exists() or not stl_dir.is_dir():
        sys.exit(f"Error: Provided --stl_dir '{stl_dir}' does not exist or is not a directory.")
